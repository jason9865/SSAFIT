<template>
  ...
  const initMap = function () {
    ...
    const mapOption = {
        center : new kakao.maps.LatLng(37.5006744185994, 127.03646946847),
        level:3
    };
    ...
    // 인포윈도우; 장소를 검색했을 때 장소명 표출
    infowindow = new kakao.maps.InfoWindow({zIndex:1});

    // 키워드로 장소를 검색
    // searchPlaces()
    const searchForm = document.getElementById("submit_btn"); // submit_btn id를 가진 button을 변수 등록
    searchForm.addEventListener("click",function(e) { //
        e.preventDefault() // 기존 submit을 막고
        searchPlaces(); // searchPlaces를 찾아준다
    });
